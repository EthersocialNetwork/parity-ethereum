...
pub use self::failsafe_guard::{RequestGuard, Error as RequestError};
...
// The backoff mechanism is not used because once a failure is reached then drop it
type ResponsePolicy = failsafe::failure_policy::SuccessRateOverTimeWindow<failsafe::backoff::Constant>;

/// The default number of OnDemand query attempts until a request is dropped 
pub const DEFAULT_QUERY_ATTEMPTS: usize = 10;
/// The initial request interval for OnDemand queries 
pub const DEFAULT_MIN_REQUEST_INTERVAL: Duration = Duration::from_secs(10);
/// The maximum request interval for OnDemand queries
pub const DEFAULT_MAX_REQUEST_INTERVAL: Duration = Duration::from_secs(100);
/// The default success rate of a time window that must be met otherwise it is a failure
pub const DEFAULT_RESPONSE_SUCCESS_RATE: f64 = 0.5;
/// The default window length when a response is regard as a failure
...
struct Pending {
	...
	response_guard: failsafe::StateMachine<ResponsePolicy, failsafe::NoopInstrument>,
}
...
pub struct OnDemand {
	...
	num_of_queries_per_request: usize,
	initial_request_interval: Duration,
...
impl OnDemand {
	...
	fn new_test(
		cache: Arc<Mutex<Cache>>, 
		num_queries_per_request: usize,
		initial_request_interval: Duration,
		max_request_interval: Duration
	) -> Self {
		let mut me = OnDemand::new(cache, num_queries_per_request, initial_request_interval, max_request_interval);
		...
		-> Result<Receiver<PendingResponse>, basic_request::NoSuchOutput>
	{
		...
		let request_guard = RequestGuard::new(
			self.num_of_queries_per_request,
			self.initial_request_interval,
			self.max_request_interval
		);

		let policy = failsafe::failure_policy::success_rate_over_time_window(
			DEFAULT_RESPONSE_SUCCESS_RATE,
			self.num_of_queries_per_request as u32,
			DEFAULT_RESPONSE_WINDOW,
			// Not used
			failsafe::backoff::constant(Duration::from_secs(60))
		);
		...
		self.submit_pending(ctx, Pending {
			...
			sender,
			request_guard,
			response_guard: failsafe::StateMachine::new(policy, failsafe::NoopInstrument)
		});
	...
	fn dispatch_pending(&self, ctx: &BasicContext) {
		...
		*pending = ::std::mem::replace(&mut *pending, Vec::new())
			...
			.filter_map(|mut pending| {
				...
				match pending.request_guard.err_call() {
					// Drop the request
...
impl Handler for OnDemand {
	...
	fn on_responses(&self, ctx: &EventContext, req_id: ReqId, responses: &[basic_request::Response]) {
		...
		if responses.is_empty() {
			// register the call as bad
			pending.response_guard.on_error();
			...
			if !pending.response_guard.is_call_permitted() {
				pending.no_response();
				return;
			}
		}
